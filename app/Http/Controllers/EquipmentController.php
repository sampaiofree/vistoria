...
final class EquipmentController extends Controller
{
    ...
    public function index(Request $request, TenantContext $tenant): InertiaResponse
    {
        ...
        $equipments = Equipment::query()->forOrganization($tenant->id())
            ...
            ->through(fn (Equipment $equipment): array => [
                'public_id' => $equipment->public_id, 'maintenance_item_code' => $equipment->maintenance_item_code, 'tag' => $equipment->tag,
    ...
    public function show(TenantContext $tenant, Request $request, Equipment $equipment, EquipmentRevisionChronology $chronology): InertiaResponse
    {
        ...
        return Inertia::render('Equipments/Show', [
            'equipment' => $this->equipmentSummaryPayload($equipment), 'client' => ['name' => $equipment->client->name], 'history_entries' => $chronology->forEquipment($equipment)->all(), 'revision_emission_types' => EquipmentRevisionEmissionType::options(),
            'can' => ['update' => $request->user()->can('update', $equipment), 'manage_revisions' => $request->user()->can('create', [EquipmentRevision::class, $equipment])],
            'index_url' => route('equipments.index'), 'edit_url' => route('equipments.edit', $equipment), 'revision_store_url' => route('equipments.revisions.store', $equipment),
        ]);
    }
    ...
    {
        return [...$equipment->only(['maintenance_plan_code', 'maintenance_item_code', 'area_code', 'area_name', 'subarea_code', 'subarea_name', 'task_list_group', 'task_list_group_counter', 'public_id', 'tag', 'defect_code_prefix', 'name', 'description', 'abc_code', 'installation_location', 'status']), 'can_edit_defect_code_prefix' => $equipment->isRegistrationEditable()];
    }
}
