...
const props = defineProps({ user: Object, action: String, status_url: String, reset_password_url: String, cancel_url: String, account_type_options: Array, operational_role_options: Array });
const page = usePage();
const temporaryCredentials = computed(() => page.props.flash?.temporary_credentials ?? null);
const form = useForm({ _method: 'put', name: props.user.name, email: props.user.email, account_type: props.user.account_type, operational_role: props.user.operational_role ?? '' });
...
<template>
    <AppLayout title="Editar usuário" :subtitle="user.email">
        <section v-if="temporaryCredentials" class="mb-5 max-w-2xl rounded-2xl border border-amber-300 bg-amber-50 p-5 text-amber-950">
