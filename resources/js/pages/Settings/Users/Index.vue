...
const props = defineProps({
    ...
    operational_role_options: { type: Array, default: () => [] },
...
const form = {
    ...
    operational_role: props.filters.operational_role ?? '',
};
...
<template>
    <AppLayout title="Usuários" subtitle="Pessoas autorizadas a acessar a organização e participar das inspeções.">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <form class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_160px_220px_180px_auto]" @submit.prevent="search">
                ...
                <select v-model="form.account_type" class="rounded-xl border border-slate-300 px-3 py-2.5 text-sm"><option value="">Todos os tipos de acesso</option><option v-for="option in account_type_options" :key="option.value" :value="option.value">{{ option.label }}</option></select>
                <select v-model="form.operational_role" class="rounded-xl border border-slate-300 px-3 py-2.5 text-sm"><option value="">Todos os papéis</option><option v-for="option in operational_role_options" :key="option.value" :value="option.value">{{ option.label }}</option></select>
        ...
        <section class="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="divide-y divide-slate-100 md:hidden">
                <article v-for="user in users.data" :key="user.public_id" class="p-5">
                    <div class="flex items-start justify-between gap-3"><div><h2 class="font-semibold text-slate-950">{{ user.name }}</h2><p class="mt-1 text-sm text-slate-500">{{ user.email }}</p></div><span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="user.status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'">{{ user.status_label }}</span></div>
                    <p class="mt-3 text-xs text-slate-500">{{ user.account_type_label }} · {{ user.operational_role_label }}<span v-if="user.open_inspections_count"> · {{ user.open_inspections_count }} inspeção(ões) aberta(s)</span></p>
                    <div class="mt-4 flex gap-2"><Link :href="user.edit_url" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold">Editar</Link><button v-if="!user.is_current_user" type="button" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold" @click="changeStatus(user)">{{ user.status === 'active' ? 'Inativar' : 'Reativar' }}</button></div>
                </article>
            </div>
            <div class="hidden overflow-x-auto md:block"><table class="min-w-full text-left text-sm"><thead class="bg-slate-50 text-xs uppercase tracking-wider text-slate-500"><tr><th class="px-5 py-3">Usuário</th><th class="px-5 py-3">Tipo de acesso</th><th class="px-5 py-3">Papel operacional</th><th class="px-5 py-3">Status</th><th class="px-5 py-3">Inspeções abertas</th><th class="px-5 py-3"></th></tr></thead><tbody><tr v-for="user in users.data" :key="user.public_id" class="border-t border-slate-100"><td class="px-5 py-4"><p class="font-semibold text-slate-950">{{ user.name }}</p><p class="text-xs text-slate-500">{{ user.email }}</p></td><td class="px-5 py-4 text-slate-600">{{ user.account_type_label }}</td><td class="px-5 py-4 text-slate-600">{{ user.operational_role_label }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="user.status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'">{{ user.status_label }}</span></td><td class="px-5 py-4 text-slate-600">{{ user.open_inspections_count }}</td><td class="px-5 py-4 text-right"><div class="flex justify-end gap-2"><Link :href="user.edit_url" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold">Editar</Link><button v-if="!user.is_current_user" type="button" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold" @click="changeStatus(user)">{{ user.status === 'active' ? 'Inativar' : 'Reativar' }}</button></div></td></tr><tr v-if="!users.data.length"><td colspan="6" class="px-5 py-10 text-center text-slate-500">Nenhum usuário encontrado.</td></tr></tbody></table></div>
